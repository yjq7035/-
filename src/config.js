...
//
// ============================================================================
// 光环通道（AURA_CHANNEL）—— 2026-09-20 新增，修「光环 / 共享资源」互相顶掉的 bug
// ----------------------------------------------------------------------------
// 为什么需要"通道"：auraManager 的记账规则是【同一目标 + 同一属性键只留一个最强来源】
//   （这样两座梯塔不会把攻速叠成两份）。但十字塔「共享资源」发出去的键**与梯塔/菱形塔
//   完全同名**（攻速 / 穿透 / 暴击率 / 破解 / 暴击伤害），于是两者在同一把键上竞争：
//   谁阶段高谁留，输的那个**整条光环凭空消失** —— 表现就是
//   「只有星级压住共享资源时，光环才给得上；否则梯塔/菱形塔等于白放」。
// 分通道之后：竞争只在**同一通道内部**发生（梯塔 vs 梯塔、十字塔 vs 十字塔），
//   不同通道的同一个键在读取时**相加**（见 auraManager.getAuraValue / getAura）。
// 通道写在塔型表（TOWER_STATS[type].auraChannel），与 auraMode 同一处登记；
//   不写 = 默认 'aura'。
// ============================================================================
const AURA_CHANNEL = {
  AURA: 'aura',    // 辅助塔自带的固有光环（梯塔攻速 / 菱形塔穿透）
  SHARE: 'share',  // 十字塔「共享资源」（把自身宝石属性按比例转给邻塔）
};
...
const TOWER_STATS = {
  ...
  //   auraChannel: SHARE = 发出去的是「共享资源」通道，与梯塔/菱形塔的固有光环**并行生效**，
  //     不再在同一把属性键上互相顶掉（见上方 AURA_CHANNEL 的说明）。
  cross:         { damage: 0, range: 0, attackSpeedMultiplier: 0, critChance: 0, critMult: 1, penetration: 0, isSupport: true, shareRatio: 25, auraMode: 'adjacent', auraChannel: AURA_CHANNEL.SHARE, description: '辅助塔，本身不提供任何攻击与属性。把自身（嵌入宝石）的属性按 25% 共享给上下左右四格的相邻塔；自身每进阶一星比例 +100%，强化「共享资源」还能继续抬高比例。' },
...
const LEVELS = (() => {
  const MAX_LEVEL = 99;
  const PLAYABLE_LEVELS = 20;  // 1~20 可玩
  ...
  const levelDescs = [
    ...
    '深渊之门开启，敌人的 HP 开始额外增长，每波比上一波多 1%。',
    '深渊之力渗透战场，怪物生命继续以额外增长推进。',
    '永恒战场，波次 11 起额外增长锁死——不再继续膨胀。',
    '最终防线：所有额外增长已封顶，考验你的策略极限。',
    '毁灭降临，怪物 HP 已达最大额外增长，每一波都是硬仗。',
    '末日黎明——怪物生命不再增长，但波次仍在继续，你能撑到最后一波吗？',
    '虚空王座：最终 4 个关卡之一，全部额外增长已锁死。',
    '创世之痕：考验最终阵容，没有额外的怪物生命膨胀，只有纯粹的波次推进。',
    '终焉之战：最终 BOSS 战，所有成长已封顶，最后的决战。',
    '永恒归途：通关全部 20 个关卡后的隐藏结局关卡。',
  ];
  const list = [];
...
const GEM_FAMILIES = [
  ...
  { id: 'amethyst', family: '紫晶宝石',  attr: 'skillEffectPercent',    label: '技能效果', unit: '%',  base: 5,  step: 5,  hue: 265, sat: 80,
  ...
  // ⚠️ neg:true = 这一族的效果是**减益**，文案必须写「-3%」而不是「+3%」。
