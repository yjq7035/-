...
class Game {
   ...
   *  - ★ 通道（2026-09-20 新增）：竞争**只在同一通道内部**发生。十字塔「共享资源」登记在
   *    'share' 通道，梯塔/菱形塔的固有光环登记在 'aura' 通道 —— 两者发出去的键虽然同名
   *    （攻速 / 穿透 / 暴击率 / 破解 / 暴击伤害），但**跨通道相加而不再互相顶掉**。
   *    通道由塔型表声明（TOWER_STATS[type].auraChannel，缺省 = 'aura'），见 config.AURA_CHANNEL。
   *    历史 bug：不分通道时，星级高的那一方会把另一方的光环整条顶掉 ——
  ...
  applyTrapezoidAuras() {
    for (const tower of this.towers) {
      ...
      // 本塔发的是哪条通道的光环（十字塔 = 'share'，梯塔/菱形塔 = 'aura'）
      const channel = stats.auraChannel || AURA_CHANNEL.AURA;

      for (const other of this.auraTargets(tower, stats)) {
        // 同一属性键上由 auraManager 在**同一通道内**取最强来源；在范围内每帧刷新持续时间
   ...
   * ⚠️ 快照是**各来源、各通道的合并和**（getAura 里同键相加）：梯塔攻速 + 十字塔共享攻速
   *    会一起进 auraBuffs，别在这里按"取最强"过滤 —— 跨通道取最强正是 2026-09-20 修掉的那个 bug。
