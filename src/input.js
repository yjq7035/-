...
// 输入层：触摸事件处理 + 全局路由
// ----------------------------------------------------------------------------
// 路由优先级（自上而下，先命中先返回）—— 谁画在最上层谁先吃触摸，不漏给下层：
//   ① 拖放进行中 → 只更新拖拽坐标
//   ② 结算界面   → 吞掉全部触摸，只处理重新开始 / 再次挑战
//   ③ 游戏中菜单（☰）→ 只处理 返回战斗 / 放弃结束
//   ④ 顶栏 🔊 音乐开关：置顶处理（早于战前选关），因为音乐一进游戏就在响，
//      关音乐的入口不能被"还没开打"挡住。（详见 handleTouchStart 内注释）
//   ⑤ 底部导航栏（导航永远可点，除非被结算/菜单/塔属性面板遮住）
//      —— 渲染层把导航画在所有场景内容之上、但塔属性面板画在它之上（nav → panel），
//         所以面板矩形内的触摸（含盖住导航栏的那一段）必须归面板而不是切场景，
//         导航判定里用 _panelRect 让面板一步。
//         同时导航必须排在所有"会提前 return 的场景分支"之前 —— 历史上天赋场景的
//         touchend 提前 return，导致"进了天赋页就再也切不出去"，别再挪到最底下。
...
function handleTouchStart(game, e) {
  ...
  // 导航永远可点，除非被结算/菜单/塔属性面板遮住：
  //   渲染层把导航画在所有场景内容之上、但塔属性面板画在它之上（nav → panel 顺序），
  //   所以手指落在属性面板矩形内（含盖住导航栏的那一段）时，导航必须让路给面板，
  //   否则点面板却切了场景 —— 与玩家眼里"面板在最上层"相反。
  // ⚠️ 这里必须排在所有"会提前 return 的场景分支"之前（见文件头）；把它挪到最底下
  //   会让 codex / talents / bag / 战前选关 等分支把导航判定挡在 return 之前，
  //   重演"进了天赋页就再也切不出去"的旧 bug。
  if (!(game.showPanel && game._panelRect && isPointInRect(pos, game._panelRect)) &&
...
function handleTouchEnd(game, e) {
  ...
  // 塔属性面板画在导航栏之上（nav → panel 渲染顺序），点落在面板矩形内
  // （含盖住导航栏的那一段）时跳过导航，交给下面的面板分支 ——
  // 面板打开时按底栏 = 面板交互（强化 / 关闭），而不是跳场景。
  if (!game.dragging && !game.pendingDrag &&
      !(game.showPanel && game._panelRect && isPointInRect(pos, game._panelRect)) &&
      pos.y >= game.H - LAYOUT.navHeight) {
    const id = nav.hitNav(game, pos);
    const item = id ? nav.NAV_ITEMS.filter((x) => x.id === id)[0] : null;
    const bp = game.btnPress;
